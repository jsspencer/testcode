...
testcode2 is a python module for testing for regression errors in numerical
(principally scientific) software.  Essentially testcode2 runs a set of
...
testcode2 requires python 2.4-3.2.  If these are not available, then `pypy
...
testcode2 can run a set of tests and check the calculated data is within a the
...
testcode2 provides access to these features via an API.  The supplied
